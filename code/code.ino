...
// 重启模组
void resetModule() {
  Serial.println("⚠️ 检测到模组异常，执行硬重启 (Power Cycle)...");

  // 1. 直接执行硬件断电重启 (最可靠)
  // 假设你已经定义了 modemPowerCycle() 函数来操作 EN 或 PWRKEY 引脚
  modemPowerCycle(); 

  // 2. 清理串口缓存 (防止上电时的乱码干扰后续判断)
  while (Serial1.available()) Serial1.read();
  
  // 3. 等待模组启动并响应 AT (握手)
  Serial.println("等待模组启动(握手)...");
  bool ok = false;
  // 模组冷启动需要时间，给它多一点时间 (例如 20秒)
  unsigned long start = millis();
  ...
  if (ok) {
    Serial.println("✅ 模组已恢复响应");
    // 这里不需要再发 AT+CFUN=1,1 了，因为硬重启本身就是一次最彻底的重启
  } else {
    Serial.println("❌ 模组硬重启后仍无响应，请检查硬件接线或供电！");
  }
}
